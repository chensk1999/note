...
**省流**：Linux是Unix的精神续作，Linux内核+组件构成Linux发行版（组件有很多人在做，没有统一版本）。市面上有五花八门的Linux发行版，Ubuntu、CentOS、ArchLinux等都是Linux发行版

**补充（操作系统内核的功能）**：进程管理、内存管理、文件系统、网络通信、安全机制、用户界面、驱动程序
