...
其中，包括了4个设计参数（$I_D$，$W$，$L$，$V_{gs} - V_{th}$）、1个约束方程，因此一个MOS管有3个自由参数。考虑这些参数与MOS管性能之间的关系，MOS管的跨导效率$\frac{g_m}{I_D} = \frac{2}{V_{ov}}$，本征频率$f_T = \frac{1}{2\pi} \frac{g_m}{C_{gs}} = \frac{1}{2\pi} \frac{2 \mu V_{ov}}{3 L^2}$。可见，过驱动电压$V_{ov}$同时控制MOS管两大重要参数，只需调节$V_{ov}$就能在两者中进行折中
