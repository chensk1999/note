...
Mugen的控制器上限是512个，但是，处于`hitpausetime`（击中对手时的停顿）时即使超过512个也不报错，多出来的控制器会覆盖原本内存的内容。第500~600个控制器的内存区域刚好是自己的角色参数，配置控制器参数可以给自己锁血，刷攻击等；大约7000多个控制器的内存是对手的参数，可以将对手`alive`设置0
...
两者相比**HSV空间更常用**，因为HSV调高亮度时颜色鲜艳程度不会漂移、降低饱和度时颜色向白色过渡，符合颜色稀释时的生活常识；CSS原生支持HSL，因此网页前端用HSL较多
