...
这两个漏洞利用了Mugen引擎调用剪贴板的代码缺少检查用户输入。利用`%n`漏洞可以写任意地址内存，进一步用`%f`漏洞执行任意代码。

原理：下面的代码中，`%*d`输出了宽度为48个字符的数字0，`%n`向`0x4b4048`地址写入48，实现了任意写内存

```c
printf("%*d%n%d", 48, 0, 0x4b4048);   // %n漏洞：覆盖内存
```

补充：此漏洞属于字符串格式化漏洞。因为`pintf`函数族不检查参数合法性，很多软件都有类似的漏洞，并且有很多利用方法，比如
