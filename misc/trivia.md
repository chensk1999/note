...
`%n`漏洞原理：printf等打印函数中，`%n`将已经打印的字符数量写到该地址。一般的编译选项禁用`%n`，但Mugen不知为何允许使用。下面的代码中，`%*d`输出了宽度为48个字符的数字0，`%n`向`0x4b4048`地址写入48，实现了任意写内存
...
2. 人眼对绿光比其他颜色更敏感，因此即使是明度、亮度相同的颜色，色相更靠近绿色的看起来更亮（也有人用灰度$Gray = 0.3 \ R + 0.6 \ G + 0.1 \ B$，表示颜色明亮程度）
...
1. **加权求和**：$\mathbf{z = W x + b}$，其中x是m维输入向量，z是n维输出向量；W是$n \times m$维的权值矩阵，b是m维偏置向量。其中，W和b可以任意调节
...
那么，隐藏层的输出就是$\mathbf{a}(x) = \phi(\mathbf{W_1} x + \mathbf{b_1})$；输出层的输出是$y = \mathbf{W_2 a} + b2$。假设固定隐藏层的权值和偏置，调整输出层的$\mathbf{W_2}$和$b_2$，这时第二层的作用就是以$\mathbf{a}(x)$作为基函数进行曲线拟合！如果再去调整隐藏层的权值，就是在调整拟合使用的函数系！

这也是神经网络的数学本质——**函数拟合**。将现实问题以抽象为数学意义上的函数，就能用神经网络拟合这个函数。大部分现实问题，哪怕是一般人都能轻松解决的问题，经过数学抽象后，都复杂到人脑无法理解、无法想象，但这丝毫不妨碍我们用神经网络拟合这个复杂得不可思议的函数
...
例如，训练一个判断图片中有没有人脸的神经网络时，我们定义输出$y$是“图中有人脸的概率”，即，$y = 0$代表图中不可能有人脸，输出$y = 1$代表图中肯定有人脸。用一张有人脸的图作为参数，我们希望神经网络输出$\hat{y} = 1$，但实际输出为0.8，那么损失就是$\mathcal{L} = (1 - 0.8)^2 = 0.04$

训练过程通常使用**梯度下降法**。对于给定的输入，输出$y$是网络权值W的函数，因此$\mathcal{L}$也可以写作W的函数$\mathcal{L}(W)$。采用一阶近似，可得在这一点附近$\mathcal{L}(W+dW) = \mathcal{L}(W) + \frac{\partial \mathcal{L}}{\partial W} dW$，让W沿着梯度反方向移动时$\mathcal{L}$下降最快，因此将W调整为$W' = W - \eta \frac{\partial \mathcal{L}}{\partial W}$

计算梯度使用了**反向传播算法**，其实就是用链式法则计算$\frac{\partial \mathcal{L}}{\partial W}$

需要注意，梯度下降不保证能找到最优解。沿着梯度寻找最低点时，完全有可能陷进局部极小值，而找不到全局极小值。工程中一般假如一点随机性以避免陷入局部极小值。而且实践告诉我们，陷入局部极小值的概率微乎其微。有理论认为，只有每个维度一阶导都为0、二阶导都大于0的点才是局部最小值，维度较高时这么巧合的事情比较少见

此外，让模型收敛到极小值时会带来过拟合风险。$\mathcal{L}$是模型参数和输入的函数$\mathcal{L}(W, x)$，前文讨论中忽略了$x$的变化，但如果针对某个$x$一直梯度下降到最小值，改变$x$后这个模型几乎必然不管用
