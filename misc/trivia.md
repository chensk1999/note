...
注意：没有完全不送气的音（因为完全不送气就只能发咂嘴声），只有相对的送气强弱。更详细的定量分析可以参见后文VOT部分
...
`%n`漏洞原理：`printf`等打印函数中，`%n`将已经打印的字符数量写到该地址。一般的编译选项禁用`%n`，但Mugen不知为何允许使用。例如下面的代码中，`%*d`输出了48个`0`字符，`%n`向`0x4b4048`地址写入48，实现了任意写内存
...
- 布洛芬：消炎止痛、退热
- 对乙酰氨基酚：止痛、退热
- 复方药：以乙酰氨基酚、咖啡因、氯苯那敏作为主要有效成分。对乙酰氨基酚和氯苯那敏能够缓解发热、头痛、流鼻涕、鼻塞等症状，同时咖啡因削弱嗜睡的副作用。市面上常见的有：复方氨酚烷胺片、氨酚咖那敏片
...
**时间轴：**参考[Netflix指南](https://partnerhelp.netflixstudios.com/hc/en-us/articles/360051554394-Timed-Text-Style-Guide-Subtitle-Timing-Guidelines)。这个指南以0.5秒作为标准，两个吸引观众注意的事件（比如对话开始与字幕出现、前一句话字幕消失到后一句话字幕切入）要么同时发生，要么相隔0.5秒以上

根据个人经验，影响力从大到小排序是`画面 > 字幕 > 语音`。因此具体规则是：

- 字幕于音频开始时切入，于音频结束0.5秒后（或者下一句话开始时）消失
- 两段字幕之间的间隔应要么等于2帧，要么大于0.5秒
- 如果对话跨越镜头变化，字幕要么与镜头同时切换，要么错开至少0.5秒

# 柯里化

柯里化（Currying）是将一个多参数函数变成嵌套的单参数函数。柯里化的用处是，有的情况下使用单一参数的闭包较为方便，有时甚至不得不使用单参数函数（例如某些函数式编程范式）。一个简单的例子是：
...
大语言模型（Large Language Model，LLM）一般指参数多于10亿的自然语言处理模型

**工作流程**

1. Tokenize：输入的语句被切分并转换为数字序列（每个数字也称作一个Token）。分词使用传统算法实现，基于频率进行
2. 模型处理Token序列（过程中会涉及注意力等机制，之后再研究），并输出一个向量，它代表下一个Token的概率分布
3. 选定一个输出Token，将它反馈回输入端，再产生下一个输出。如此重复若干次，产生输出Token序列。最后将输出Token序列转换为词语，即得到了生成的文本

从工作流程来看，LLM本质上是一个词语接龙机器。只要在输入中包含必要信息，它就能回答特定的问题。背景信息常叫做上下文（Context），具体问题则叫做提示词（Prompt）

**智能体**（Agent）：用户与LLM之间的中介，能够替LLM执行操作——比如从本地文件、网络、向量数据库获取数据，或者进行文件操作、运行外部程序。例如：
...
**MCP**（Model Context Protocol）：Anthropic公司主导的开源协议，规定了Agent与外部工具沟通的数据格式。这个设计让Agent主程序和它能调用的功能解耦

**技能**（Skill）：提前写好的工作流程，使用自然语言编写，可以附带程序、脚本。需要执行相应任务时（可以是关键词触发，也可以用类似MCP的方式调用），将它加载到上下文中
...
视频传输可分为四种场景：直播推流、视频推流、观看直播、观看点播视频。每种场景都有各自适配的协议
...
#EXTINF:2.000,    # 第三个分片
56-25_278.ts
#EXT-X-ENDLIST    # 结束符（如果是直播，结束前不会出现此行）
```

对于点播视频，客户端逐个加载分片并播放。因为每个分片都是无状态的小文件，负载均衡和CDN的加速效果较明显，拉进度条时加载也更快
...
**跨省流量**：如果网络请求的源和目的不在同一省份，就属于跨省流量。比如广东电信用户访问上海电信机房的服务器，请求包流向是广东电信接入网 → 跨省骨干网 → 上海电信接入网；返回包则反过来。这些数据包都是广东电信 → 上海电信的跨省流量。注意，上下行流量都算作广东发起的。虽然数据包是一来一回，但从业务上来说返回包是响应用户请求，因此数据包的真实用户始终是广东

**跨省结算**：跨省流量增加了运营商成本，如果两省之间流量方向不平衡则会导致收益不平衡。比如贵州省的数据中心多而用户少，网络建设与运营成本有不少都是为了给外省提供服务，因此其他省份的运营商需要向贵州运营商支付跨省流量结算费作为补偿（注意：是不同省份运营商之间结算。普通用户不需要支付）

2024年，国内三大运营商开始施行跨省结算。推行主要目的是打击PCDN，同时也严重影响了BT以及正常上传用户
...
宪法规定，我国行政区划分为省、市县、乡镇三级。实际运作中，采用4~5级的管理体系，具体划分方式因地制宜，在法律规定基础上打了大量补丁
...
1983~1997年，为发展城市拉动经济，实施**撤县改市**，撤销县建制，在原行政区域设立县级市。改革初期有一定效果，但后期各地盲目跟风，导致许多农业人口比例大、城镇比重不足的县也改为县级市，导致虚假城市化、城乡界限混乱。此后县改市被严格控制
